...
class Utils:
    def __init__(self, models_path="models", tmp_path="tmp"):
        for dir_path in [models_path, tmp_path]:
            if not path.exists(dir_path):
                raise ValueError(dir_path, "doesn't exist.")
            elif not path.isdir(dir_path):
                raise ValueError(dir_path, "isn't a directory.")
        self.models_path = models_path
        self.tmp_path = tmp_path

        self.__mcc = MouseConnectivityCache()
        self.__mca = MouseConnectivityApi()
        self.__st_tree = self.__mcc.get_structure_tree()
        self.__image_api = ImageDownloadApi()
        self.__dict_struct_name_id = self.__st_tree.get_name_map()
        self.__dict_struct_name = {v.upper(): k for k, v in self.__dict_struct_name_id.items()}
        self.__dict_struct_acron = {k.upper(): v for k, v in self.__st_tree.get_id_acronym_map().items()}
        self.__dict_struct_acron_id = {v: k for k, v in self.__dict_struct_acron.items()}

        cre_neg_exp = self.__mcc.get_experiments(dataframe=True, cre=False)
        cre_neg_exp['cre'] = np.zeros((len(cre_neg_exp), 1), dtype=bool)
        cre_pos_exp = self.__mcc.get_experiments(dataframe=True, cre=True)
        cre_pos_exp['cre'] = np.ones((len(cre_pos_exp), 1), dtype=bool)
        self.all_exp = pd.concat([cre_neg_exp, cre_pos_exp]).round(5)

    def __nrrd_to_nifti(self, nrrd_array, res):
        affine = np.zeros((4, 4))
        affine[0, 2] = res * 1e-3
        affine[1, 0] = -res * 1e-3
        affine[2, 1] = -res * 1e-3
        ...
        file_path = self.tmp_path + "/" + "nifti_volume_" + str(res) + "_" + uuid.uuid4().hex + ".nii"
        nib.save(nifti_volume, file_path)

        return file_path

        # get all structures in a dict (key: id, value: name with acronym)
    def get_struct_in_dict(self):
        st_dict = {}

        for i in range(0, len(self.all_exp)):
            index_val = self.all_exp.iloc[i]
            struct_ids = index_val['injection_structures']

            for struct_id in struct_ids:
                if struct_id not in st_dict:
                    struct_dict = self.__st_tree.get_structures_by_id([struct_id])[0]
    ...
    def get_node_list(self):
        ...
        for i in range(0, len(self.all_exp)):
            ...
            for struct_id in struct_ids:
                ...
                if struct_id not in node_list:
                    ...
                    ancestor_ids_list = self.__st_tree.ancestor_ids([struct_id])
        ...
        node_dictionaries = self.__st_tree.nodes(node_list)
    ...
    def get_structures_childs(self, acronyms, names):
        '''
        ...
        if acronyms is not None:
            for struct in acronyms:
                ...
                if struct_uppercase in self.__dict_struct_acron:
                    structure_childs = self.__st_tree.descendant_ids([self.__dict_struct_acron[struct_uppercase]])[0]
                    structures_childs_id.extend(child_id for child_id in structure_childs
                                                if child_id not in structures_childs_id and
                                                child_id != self.__dict_struct_acron[struct_uppercase])

        if names is not None:
            for struct in names:
                struct_uppercase = struct.upper().strip(" ")
                if struct_uppercase in self.__dict_struct_name:
                    structure_childs = self.__st_tree.descendant_ids([self.__dict_struct_name[struct_uppercase]])[0]
                    structures_childs_id.extend(child_id for child_id in structure_childs
                                                if child_id not in structures_childs_id and
                                                child_id != self.__dict_struct_name[struct_uppercase])

        # structure ids to acronyms
        structures_childs = []
        for struct_id in structures_childs_id:
            structures_childs.append(self.__dict_struct_acron_id[struct_id])

        return structures_childs

    def get_experiments_csv(self, experiment_ids):
        filter_dict = {}
        errors = []

        if experiment_ids is not None:
            for exp_id in experiment_ids:
                if exp_id in self.all_exp.index:
                    filter_dict[exp_id] = self.all_exp.loc[exp_id]
                else:
    ...
    def get_experiment_volume(self, experiment_id, img_type, resolution):
        '''
        ...
        if img_type in allowed_img_types:
            ...
            # download nrrd files
            save_file_path = self.tmp_path + f"/{experiment_id}_{img_type}_{resolution}.nrrd"
            ...
            return self.__nrrd_to_nifti(nrrd.read(save_file_path)[0], resolution), errors
        else:
            errors.append(img_type + " image type doesn't exist")
        return None, errors

    def get_average_projection_density(self, experiment_ids, resolution):
        p_mcc = MouseConnectivityCache()
        p_mcc.resolution = resolution  # [10. 25. 50. 100]
        errors = []

        template = p_mcc.get_template_volume(file_name=self.models_path + "/average_template_" +
        ...
        if valid_experiment_ids is not None:
            for exp_id in valid_experiment_ids:
                vol_avg += p_mcc.get_projection_density(
                    ...
                    file_name=self.tmp_path + "/" + str(exp_id) +
                              "_projection_density_" +
                              str(p_mcc.resolution) + ".nrrd"
                )[0] / len(valid_experiment_ids)

        file_path = self.__nrrd_to_nifti(vol_avg, p_mcc.resolution)

        return file_path, errors

    def get_streamlines(self, experiment_ids):
        sapi = streamlines.StreamLines(directory=self.tmp_path)
        ...
        if experiment_ids:
            ...
            for exp_id in experiment_ids:
                if exp_id not in self.__mcc.get_experiments(dataframe=True)['id']:
        ...
        if len(sapi.data) > 0:
            ...
            tractogram_path = self.tmp_path + "/streamlines_" + uuid.uuid4().hex + ".trk"
    ...
    def get_template(self, resolution):
        p_mcc = MouseConnectivityCache()
        p_mcc.resolution = resolution  # [10. 25. 50. 100]
        ...
        return self.__nrrd_to_nifti(template, p_mcc.resolution)

    def __validate_structures(self, structures, errors, category):
        if structures is not None:
            for i in range(0, len(structures)):
                ...
                if struct.isdigit():
                    struct = int(struct)
                    structures[i] = struct
                    if struct not in self.__dict_struct_name_id:
                        exist = False
                elif struct in self.__dict_struct_name:
                    structures[i] = self.__dict_struct_name[struct]
                elif struct not in self.__dict_struct_acron:
                    exist = False
                if not exist:
                    errors.append(category + " |" + str(struct) + "| doesn't exist")
        return

    def __validate_hemisphere(self, hemisphere, errors, category):
    ...
    def correlation_search(self,
        ...
        if row not in self.__mcc.get_experiments(dataframe=True)['id']:
            errors.append("experiment |" + str(row) + "| doesn't exist")

        self.__validate_structures(structures=structures,
                                   errors=errors,
                                   category="structure")

        self.__validate_structures(structures=injection_structures,
                                   errors=errors,
                                   category="injection structures")

        self.__validate_hemisphere(hemisphere=hemisphere,
                                   errors=errors,
                                   category="hemisphere")
        ...
        result = self.__mca.experiment_correlation_search(row=row,
    ...
    def injection_correlation_search(self,
                                     seed_point,                   # [Integer]
                                     product_ids=None,             # [Integer], None
                                     transgenic_lines=None,        # [String], None, Specify ID '0' to exclude all TransgenicLines.
                                     injection_structures=None,    # [String], None
        ...
        result = self.__mca.experiment_injection_coordinate_search(seed_point=seed_point,
                                                                   injection_structures=injection_structures,
                                                                   product_ids=product_ids,
                                                                   transgenic_lines=transgenic_lines,
                                                                   primary_structure_only=primary_structure_only,
    ...
    def source_search(self,
                      injection_structures,         # [String], None
        ...
        self.__validate_structures(structures=target_domain,
                                   errors=errors,
                                   category="target domain")

        self.__validate_structures(structures=injection_domain,
                                   errors=errors,
                                   category="injection domain")

        self.__validate_hemisphere(hemisphere=injection_hemisphere,
                                   errors=errors,
                                   category="injection hemisphere")

        self.__validate_hemisphere(hemisphere=target_hemisphere,
                                   errors=errors,
        ...
        result = self.__mca.experiment_source_search(injection_structures=injection_structures,
    ...
    def hotspot_search(self,
                       rows,                  # [Integer]
                       injection_structures,  # [String, Integer], None
                       depth):                # String
        errors = []

        all_exp_id = self.__mcc.get_experiments(dataframe=True)['id']
        for row in rows:
        ...
        if injection_structures is not None:
            for struct in injection_structures:
                structure_id = None
                if struct in self.__dict_struct_name_id:
                    structure_id = struct
                elif struct in self.__dict_struct_acron:
                    structure_id = self.__dict_struct_acron[struct]
                elif struct in self.__dict_struct_name:
                    structure_id = self.__dict_struct_name[struct]
                else:
                ...
                if depth == 'child':
                    structure_childs = self.__st_tree.child_ids([structure_id])[0]
                    structures_id.extend(struct for struct in structure_childs
                                         if struct not in structures_id)
                elif depth == 'all':
                    structure_descendants = self.__st_tree.descendant_ids([structure_id])[0]
        ...
        # connectivity matrix
        pm = self.__mcc.get_projection_matrix(experiment_ids=rows,
